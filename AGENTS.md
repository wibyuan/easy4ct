...
2. **证明起止标记**：以加粗 `**证明：**` 开始，`**证明：**` 之后换行留空行再接证明内容；以黑色方块 `$\blacksquare$` 结束，`$\blacksquare$` 紧跟证明最后一句之后（同行），且其后换行留空行再接后续内容。多情形证明可用 `**证明1（xxx）：**`、`**证明2（xxx）：**` 开始，同行直接接证明内容，各自以 `$\blacksquare$` 结束。**证明块内不得嵌套任何引用块（定义/定理/引理/命题），也不得嵌套任何证明块**；引用块和证明块须平级排列。**定理与其对应的证明块之间不得嵌套任何引用块或其它证明块**。
