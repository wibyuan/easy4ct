...
1. **定义/定理用引用块**：`> **定义（名称）**` 或 `> **定理（名称）**` 开头，条目名加粗，内容为引用块内若干行。
2. **证明起止标记**：以加粗 `**证明：**` 开始，以黑色方块 `$\blacksquare$` 结束；多情形证明可用 `**证明1（xxx）：**`、`**证明2（xxx）：**`。
3. **图注**：图片后允许空行再接图注，解释绘图的基本意义，以"如图"为提示起始语（暂不编号，合并时统一编号）。图注不加数学公式渲染（避免喧宾夺主、取代正文），以纯文本辅助理解为主，严谨性由正文保证；后续转 LaTeX 时可按需补充公式渲染。
4. **暂不加引用、总结、概括或导语**：撰写完成后再统一分配。
5. **长文拆分**：markdown 过长时拆分为多个文件（按文章章节），便于人类编辑；最终转 LaTeX + TikZ PDF 时再合并。
