...
> **定义（分离圈）**
>
> 如果一个平面图中存在一个很小的圈，它的内部和外部都有顶点，那么这个构型称作**分离圈**，其中长度为 $k$ 的称作 **$\boldsymbol k$ - 分离圈**。
...
Birkhoff 则在 1913 年证明了 4-分离圈和内部和外部分别至少有两个顶点与圈连通的 5-分离圈是可约的[7]，注意“内部和外部分别至少有两个顶点与圈连通”的限制，不然可能会造成不必要的误解，比如以为 Birkhoff 证明了 5 度顶点可约并终结了四色定理，并没有。
...
自然，这个结论是需要证明的。由于顶点数大于 3 的极大平面图，任意一个极小点割集，其导出子图总是一个圈。又由于已经证明了最小反例不存在 3-分离圈，4-分离圈，内外顶点个数不小于 2 的 5-分离圈；且极大平面图的连通度至少是 3。
