...
> **定义（平面图）**
>
> 一个图是**平面图**当且仅当它可以被边不交叉地画在平面上。

> **定义（平面嵌入）**
>
> 把一个平面图边不交叉地画在平面上，这种画法称作该平面图的**平面嵌入**。

> **定义（合法面染色）**
>
> 对平面图的平面嵌入的每个面（包括最外围的无限大区域，即外部面）进行染色，使得相邻两个面颜色不同，则称其为该平面嵌入的**合法面染色**。

如果两个面仅在对角线处相碰，如下图，它们不被视为相邻，此时它们也可以染成同一种颜色。

> **定义（面色数）**
>
> 一个平面图的平面嵌入的合法面染色需要用到的最少颜色数，称为该平面嵌入的**面色数**。
