...
当前阶段：已收录并跑通 RSST 1997 验证引擎（633 构型），四色定理文章按 res-plan 分块推进，全部底稿已完稿待审：`res-01`~`res-06a`（已完成，作者重写 + 助手续写）；第六部分（可约性判定）`res-07c`~`res-07h`、第七部分（放电法，沿用 res-07 系列编号）`res-07i`/`res-07j`、第八部分 `res-08`（完整证明过程与染色算法）、第九部分 `res-09`（交互式证明器），另加 `res-07k`（reduce.c/discharge.c 逐函数代码导览）——均待作者审阅。自检轮已完成：格式与交叉引用检查通过；(6.3) 4 圈分类改为程序穷举核验（`rsst/docs/verify-consistent4.py` 入库，14 相似类中 9 个非空一致集全部含四并集之一）、相似类数修正为 14（Burnside）、(6.4) 子断言二与 res-07j 概述项补全。`res-08-参考.md` 已改名 `res-ref-参考.md`。术语体系约定见 AGENTS.md 规则 17。RSST 引理 (4.1) 的字面表述疏漏的完整判定见 `rsst/docs/RSST-(4.1)-字面失效范围刻画底稿.md`。
