...
当前阶段：已收录并跑通 RSST 1997 验证引擎（633 构型），正在撰写四色定理文章（已完成章节拆分，确立定义/定理引用块、证明起止标记、伪证标记、定义-定理分工、术语首次使用检查与术语引入前置检查等格式规范）。`res-06a`（顶点邻域的双圈结构）已由作者重新起草并续写完成；`res-07`（可约性判定）按块拆分推进，`res-07a`（标准构型与自由补全）术语体系已定（RSST configuration 译为标准构型，可约构型/局部构型保留）并经作者逐项修正（全度数、帽带、帽齿、专属齿、充盈顶点、边界段、准三角剖分等），残留已清；`res-07b`（环染色与一致集）判定需重构；助手对正文提出改善意见的角色已终止。术语体系约定见 AGENTS.md 规则 17（术语引入前置检查：来源论文术语译名以文章已有译名为准，新词须候选清单交作者确认）。RSST 引理 (4.1) 的字面表述疏漏的完整判定见 `rsst/docs/RSST-(4.1)-字面失效范围刻画底稿.md`。
